...
def buscar_nombre(parte_nombre):
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("SELECT Titulo FROM Presentacion WHERE Titulo LIKE %s", ('%{}%'.format(parte_nombre),))
            resultados = cursor.fetchall()
            return resultados
    except Exception as e:
        print("Error al buscar dato en esclavo3", e)
        return None
    finally:
        conn.close()
...
def buscar_titulo():
    ...
    print("El titulo del esclavo3: ",titulo)
    if titulo:
...
def buscar_tipo():
    ...
    if tipo:
        ...
        try:
            with conn.cursor() as cursor:
                cursor.execute("SELECT * FROM presentacion")
                resultados = cursor.fetchall()
                return jsonify(resultados)
        except Exception as e:
            print("Error al buscar dato en esclavo3", e)
